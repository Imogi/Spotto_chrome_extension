...
/*
    client id, state and access token
*/
...
*/
function create_authorisation_endpoint() {
...
async function getAC_TOKEN(key) {
...
async function callApi(method, url, body, callback) {
    ...
    let AC_TOKEN = await getAC_TOKEN('AC');
...
function generateEndpointAddSongToPlaylist(play_list_id) {
    ...
    // Issue is if the user refreshes popup the recent stored data dissapears. I.e does not persist
    RECENT_PLAYLIST_ID = play_list_id;
    RECENT_SONG_ID = SONG_URI;
...
function generateEndpointDeleteSongFromPlaylist() {

    // If there is no recent playlist
    if (RECENT_PLAYLIST_ID == '') {
        // Modal popup
    ...
    // Else if there is a recent playlist
        ...
        let ItemAddEndpoint = "https://api.spotify.com/v1/playlists/";
        // Add cached playlist i.e the playlist that has recently been added to
        ItemAddEndpoint += RECENT_PLAYLIST_ID;
        ItemAddEndpoint += "/tracks";
        return ItemAddEndpoint;
    }
}

function generateRequestBodyForRemovingSongs() {
    let body = "{\"tracks\":[{\"uri\":\"" + RECENT_SONG_ID + "\"}]}";
    return body;
}

// Calls the callApi function to remove song from a playlist
function RemoveItemFromPlaylist(ItemAddEndpoint, request_body) {
    callApi("DELETE", ItemAddEndpoint, request_body, handleRemoveSongFromPlaylistResponse);
}
...
function handlePfpResponse() {
    if (this.status == 200) {
        ...
        if (li != null) {
            li.appendChild(img);
            //header.appendChild(display_name);
        }
    }
